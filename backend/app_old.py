...
from models import ( # Убрана точка
    ...
    NotificationAction, NotificationType, NotificationRecipient, NotificationStatus
)
from notification_service import NotificationService, NotificationConfig # Убрана точка
            ...
            try:
                ...
                notifications = loop.run_until_complete(
                    notification_service.create_appointment_notification(
                        session, appointment.id, NotificationType.appointment_created
                    )
                )
...
# Обновленный endpoint для связывания клиента с Telegram
...
# Исправленный API эндпоинт для авторизации мастеров
...
def authenticate_master():
    ...
    try:
        ...
        # Единая функция нормализации номера телефона (приводим к формату +7XXXXXXXXXX для сотрудников)
        def normalize_phone(phone_input):
            import re
            digits_only = re.sub(r'\D', '', phone_input)
